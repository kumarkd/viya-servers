...
async function run (appDirectory, version) {
	...
	let base = `npm install @kumarkd/viya-api-base@${version}`;
	console.log(`Installing @kumarkd/viya-api-base@${version}`);
    await execcmd(base);
	sh.cp('-rf', './node_modules/@kumarkd/viya-api-base/template/*', '.');

	// update package.json with the template.json
	let templatejs = await fs.readFile('./node_modules/@kumarkd/viya-api-base/template.json', 'utf8');
...
run(appDir,version)
	.then(() => {
		console.log('Please visit https://github.com/kumarkd/viya-servers/wiki/viya-apiserverjs for documentation');
	})
